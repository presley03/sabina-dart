...
class HealthAnalyticsService {
  ...
  static List<HealthInsight> analyzeWeightTrend(List<HealthRecord> records) {
    ...
    final weeksDiff = weightRecords.last.date
        .difference(weightRecords.first.date)
        .inDays / 7;
    ...
    if (weeklyChange > 0.5) {
      insights.add(HealthInsight(
        ...
        description: 'Berat badan Anda naik ${weeklyChange.toStringAsFixed(1)} kg per minggu',
        recommendation: 'Konsultasikan dengan dokter tentang pola makan dan aktivitas fisik yang tepat',
      ...
      insights.add(HealthInsight(
        ...
        recommendation: 'Pastikan asupan nutrisi mencukupi dan konsultasi dengan ahli gizi',
        type: InsightType.weight,
  ...
  static List<HealthInsight> analyzeBloodPressure(List<HealthRecord> records) {
    ...
    if (highBPPercentage > 30) {
      insights.add(HealthInsight(
        ...
        description: '${highBPPercentage.toStringAsFixed(0)}% pengukuran menunjukkan tekanan darah tinggi',
        recommendation: 'SEGERA konsultasi dengan dokter untuk evaluasi preeklampsia',
      ...
      insights.add(HealthInsight(
        ...
        recommendation: 'Monitor tekanan darah lebih sering dan konsultasi dengan dokter',
        type: InsightType.bloodPressure,
        priority: InsightPriority.high,
      ));
    }
  ...
  static List<HealthInsight> analyzeSymptoms(List<HealthRecord> records) {
    ...
    for (final record in symptomsRecords) {
      ...
      if (symptoms.contains('sakit kepala')) {
        symptomFrequency['sakit kepala'] = (symptomFrequency['sakit kepala'] ?? 0) + 1;
      }
      if (symptoms.contains('mual') || symptoms.contains('muntah')) {
        symptomFrequency['mual muntah'] = (symptomFrequency['mual muntah'] ?? 0) + 1;
      }
    ...
    symptomFrequency.forEach((symptom, count) {
      ...
      if (frequency > 50) {
        ...
        switch (symptom) {
          case 'sakit kepala':
            recommendation = 'Sakit kepala berulang bisa menjadi tanda preeklampsia. Konsultasi dengan dokter';
            priority = InsightPriority.high;
            break;
          case 'bengkak':
            recommendation = 'Bengkak berlebihan perlu dievaluasi dokter untuk kemungkinan preeklampsia';
            priority = InsightPriority.high;
            break;
          case 'mual muntah':
            recommendation = 'Mual muntah berlebihan (hyperemesis) perlu penanganan medis';
            priority = InsightPriority.medium;
            break;
          default:
            recommendation = 'Gejala berulang perlu dikonsultasikan dengan dokter';
        }
        
        insights.add(HealthInsight(
          title: 'Gejala $symptom Berulang',
          description: 'Gejala ini muncul pada ${frequency.toStringAsFixed(0)}% catatan kesehatan',
  ...
  static List<HealthInsight> analyzeSleepPattern(List<HealthRecord> records) {
    ...
    final averageSleep = sleepRecords
        .map((r) => r.sleepHours!)
    ...
    if (averageSleep < 7) {
      insights.add(HealthInsight(
        title: 'Kurang Tidur',
        description: 'Rata-rata tidur Anda ${averageSleep.toStringAsFixed(1)} jam per hari',
        recommendation: 'Ibu hamil membutuhkan 7-9 jam tidur. Coba atur jadwal tidur yang lebih teratur',
      ...
      insights.add(HealthInsight(
        ...
        description: 'Rata-rata tidur Anda ${averageSleep.toStringAsFixed(1)} jam per hari',
        recommendation: 'Tidur berlebihan bisa menandakan kelelahan atau kondisi medis tertentu',
        type: InsightType.sleep,
        priority: InsightPriority.medium,
      ));
    }
  ...
  static List<HealthInsight> analyzeHydration(List<HealthRecord> records) {
    ...
    final hydrationRecords = records
        .where((r) => r.waterIntake != null)
        .toList();
    
    if (hydrationRecords.isEmpty) return insights;
    
    final averageWater = hydrationRecords
        .map((r) => r.waterIntake!)
        .reduce((a, b) => a + b) / hydrationRecords.length;
    
    // Target: 2000-2500ml per hari
    if (averageWater < 2000) {
      insights.add(HealthInsight(
        title: 'Kurang Minum Air',
        description: 'Rata-rata asupan air ${(averageWater/1000).toStringAsFixed(1)} liter per hari',
        recommendation: 'Ibu hamil perlu 2-2.5 liter air per hari. Tingkatkan asupan air putih',
  ...
  static List<HealthInsight> analyzeMood(List<HealthRecord> records) {
    ...
    final negativeMoodPercentage = (negativeMoodCount / moodRecords.length) * 100;
    
    if (negativeMoodPercentage > 60) {
      insights.add(HealthInsight(
        title: 'Mood Negatif Dominan',
        description: '${negativeMoodPercentage.toStringAsFixed(0)}% catatan menunjukkan mood negatif',
        recommendation: 'Pertimbangkan konseling atau terapi untuk kesehatan mental selama kehamilan',
      ...
      insights.add(HealthInsight(
        ...
        recommendation: 'Lakukan aktivitas relaksasi dan bicarakan dengan orang terdekat',
        type: InsightType.general,
        priority: InsightPriority.medium,
      ));
    }
  ...
  static List<HealthInsight> generateComprehensiveInsights(List<HealthRecord> records) {
      ...
      case 1:
        return [
          HealthInsight(
            ...
            description: 'Trimester pertama adalah periode kritis untuk perkembangan saraf bayi',
            recommendation: 'Konsumsi asam folat 400mcg setiap hari dan hindari alkohol serta rokok',
          ...
          HealthInsight(
            ...
            recommendation: 'Makan dalam porsi kecil tapi sering, hindari makanan berminyak',
            type: InsightType.general,
            priority: InsightPriority.medium,
          ),
        ];
      ...
      case 2:
        return [
          HealthInsight(
            ...
            description: 'Trimester kedua adalah waktu pertumbuhan tulang dan darah bayi',
            recommendation: 'Tingkatkan asupan kalsium dan zat besi, lakukan olahraga ringan',
            type: InsightType.nutrition,
            priority: InsightPriority.high,
          ),
          HealthInsight(
            ...
            recommendation: 'Lakukan USG anomali dan tes skrining sesuai jadwal dokter',
            type: InsightType.general,
            priority: InsightPriority.medium,
          ),
        ];
      ...
      case 3:
        return [
          HealthInsight(
            ...
            recommendation: 'Siapkan tas rumah sakit, pelajari teknik pernapasan, monitor gerakan bayi',
            type: InsightType.general,
            priority: InsightPriority.high,
          ),
          HealthInsight(
            ...
            recommendation: 'Monitor tekanan darah, perhatikan bengkak berlebihan dan sakit kepala',
            type: InsightType.symptoms,
            priority: InsightPriority.high,
          ),
        ];
